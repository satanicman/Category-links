...
class LinkBlockPresenter
{
    ...
    {
        $cmsLinks = $categoryLinks = $productLinks = $staticsLinks = $customLinks = array();
     ...
     * @throws \PrestaShopException
     */
    private function makeCategoryLinks($categoriesIds)
    {
        $categoryLinks = [];
        foreach ($categoriesIds as $categoryId) {
            /** @var \CategoryCore $category */
            $category = new \Category((int) $categoryId);
            if (null !== $category->id && $category->active) {
                $categoryLinks[] = [
                    'id' => 'category-page-' . (int)$category->id,
                    'class' => 'category-link',
                    'title' => $category->name[(int) $this->language->id],
                    'description' => $category->meta_title[(int) $this->language->id],
                    'url' => $this->link->getCategoryLink($category->id, $category->link_rewrite[(int)$this->language->id]),
                ];
            }
        }
